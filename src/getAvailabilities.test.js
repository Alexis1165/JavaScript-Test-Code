...
describe("getAvailabilities", () => {
  ...
  describe("case 2", () => {
    ...
    it("Slot validity test w.r.t appointment", async () => {
        const availabilities = await getAvailabilities(new Date("2014-08-10"));
        expect(availabilities.length).toBe(7);
  ...
  describe("case 3", () => {
    ...
    it("Slot validity test w.r.t opening", async () => {
      const availabilities = await getAvailabilities(new Date("2014-08-10"));
      expect(availabilities.length).toBe(7);
      expect(String(availabilities[0].date)).toBe(String(new Date("2014-08-10")));
      expect(availabilities[0].slots).toEqual([]);
  ...
  describe("case 4", () => {
    beforeEach(async () => {
      await knex("events").insert([
        {
          kind: "appointment",
          starts_at: new Date("2020-04-15 10:30"),
          ends_at: new Date("2020-04-15 11:30")
        },
        {
          kind: "opening",
          starts_at: new Date("2020-04-06 09:30"),
          ends_at: new Date("2020-04-06 12:30"),
          weekly_recurring: true
        }
      ]);
    });

    it("Day count feature test", async () => {
      const availabilities = await getAvailabilities(new Date("2020-04-14"), 10);
      expect(availabilities.length).toBe(10);
      expect(String(availabilities[6].date)).toBe(String(new Date("2020-04-20")));
      expect(availabilities[6].slots).toEqual([
        "9:30",
        "10:00",
        "10:30",
      ...
      expect(String(availabilities[9].date)).toBe(String(new Date("2020-04-23")));
